...
// const createFile: void = fs.writeFileSync(
//   "LargeFile.txt",
...
const reader: fs.ReadStream = fs.createReadStream("./LargeFile.txt");

const writer: fs.WriteStream = fs.createWriteStream("./write.txt");
...
for (let i = 0; i <= 10; i++) {
  ...
  writeToStream.write(
    `This is the " + ${i} ${add} + " time i am writing this `
  );
}
